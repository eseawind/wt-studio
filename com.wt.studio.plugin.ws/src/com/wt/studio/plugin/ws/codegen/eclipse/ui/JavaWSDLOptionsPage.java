...
import com.wt.studio.plugin.ws.codegen.eclipse.util.NamespaceFinder;
...
public class JavaWSDLOptionsPage extends AbstractWizardPage {
	...
	public void setNamespaceDefaults(String fullyQualifiedClassName) {
		...
		targetNamespaceText.setText(NamespaceFinder
				.getTargetNamespaceFromClass(fullyQualifiedClassName));
		schemaTargetNamepaceText.setText(NamespaceFinder
				.getSchemaTargetNamespaceFromClass(fullyQualifiedClassName));

		targetNamespacePrefixText.setText(NamespaceFinder
				.getDefaultNamespacePrefix());
		schemaTargetNamespacePrefixText.setText(NamespaceFinder
				.getDefaultSchemaNamespacePrefix());

		// serviceNameText.setText(fullyQualifiedClassName.replaceAll("\\.","_"));
		serviceNameText.setText(NamespaceFinder
				.getServiceNameText(fullyQualifiedClassName));
	}
	...
	public void createControl(Composite parent) {
		...
		label = new Label(container, SWT.NULL);
		label.setText(Activator.getResourceString("page5.schemaTargetNs.label"));
		...
		label = new Label(container, SWT.NULL);
		label.setText(Activator.getResourceString("page5.serviceName.label"));

		gd = new GridData(GridData.FILL_HORIZONTAL);
		serviceNameText = new Text(container, SWT.BORDER);
		serviceNameText.setLayoutData(gd);
		serviceNameText.setText(settings.get(PREF_JAVA_SCHEMA_TARGET_NS));
		serviceNameText.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				settings.put(PREF_JAVA_SERVICE_NAME, serviceNameText.getText());
				dialogChanged();
			}
		});
