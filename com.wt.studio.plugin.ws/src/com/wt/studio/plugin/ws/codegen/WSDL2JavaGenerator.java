...
package com.wt.studio.plugin.ws.codegen;
...
public class WSDL2JavaGenerator {
	 ...
	 * 
	 * @param UILangValue
	 *            a string containg a language, e.g. "java", "cs", "cpp" or "vb"
	 ...
	 * decisions made by the user on the OptionsPage (page2). For each setting,
	 * there is a Command-Line option for the Axis2 code generator.
	 * 
	 * @return a Map with keys from CommandLineOptionConstants with the values
	 *         entered by the user on the Options Page.
	 */
	public Map fillOptionMap(boolean isAyncOnly, boolean isSyncOnly,
		...
		// WSDL file name
		optionMap
				.put(CommandLineOptionConstants.WSDL2JavaConstants.WSDL_LOCATION_URI_OPTION,
		...
		if (isAyncOnly) {
			optionMap
					.put(CommandLineOptionConstants.WSDL2JavaConstants.CODEGEN_ASYNC_ONLY_OPTION,
							new CommandLineOption(
		...
		if (isSyncOnly) {
			optionMap
					.put(CommandLineOptionConstants.WSDL2JavaConstants.CODEGEN_SYNC_ONLY_OPTION,
							new CommandLineOption(
		...
		if (isServerSide) {
			optionMap
					.put(CommandLineOptionConstants.WSDL2JavaConstants.SERVER_SIDE_CODE_OPTION,
							new CommandLineOption(
			...
			if (isServerXML) {
				optionMap
						.put(CommandLineOptionConstants.WSDL2JavaConstants.GENERATE_SERVICE_DESCRIPTION_OPTION,
								new CommandLineOption(
			...
			if (isGenerateAll) {
				optionMap
						.put(CommandLineOptionConstants.WSDL2JavaConstants.GENERATE_ALL_OPTION,
								new CommandLineOption(
		...
		if (isTestCase) {
			optionMap
					.put(CommandLineOptionConstants.WSDL2JavaConstants.GENERATE_TEST_CASE_OPTION,
							new CommandLineOption(
		...
		// package name
		optionMap
				.put(CommandLineOptionConstants.WSDL2JavaConstants.PACKAGE_OPTION,
						new CommandLineOption(
								CommandLineOptionConstants.WSDL2JavaConstants.PACKAGE_OPTION,
								getStringArray(packageName)));
		// selected language
		optionMap
				.put(CommandLineOptionConstants.WSDL2JavaConstants.STUB_LANGUAGE_OPTION,
						new CommandLineOption(
								CommandLineOptionConstants.WSDL2JavaConstants.STUB_LANGUAGE_OPTION,
								getStringArray(mapLanguagesWithCombo(selectedLanguage))));
		// output location
		optionMap
				.put(CommandLineOptionConstants.WSDL2JavaConstants.OUTPUT_LOCATION_OPTION,
						new CommandLineOption(
		...
		// databinding
		optionMap
				.put(CommandLineOptionConstants.WSDL2JavaConstants.DATA_BINDING_TYPE_OPTION,
						new CommandLineOption(
		...
		if (portName != null) {
			optionMap
					.put(CommandLineOptionConstants.WSDL2JavaConstants.PORT_NAME_OPTION,
							new CommandLineOption(
		...
		if (serviceName != null) {
			optionMap
					.put(CommandLineOptionConstants.WSDL2JavaConstants.SERVICE_NAME_OPTION,
							new CommandLineOption(
		...
		if (namespace2packageList != null) {
			optionMap
					.put(CommandLineOptionConstants.WSDL2JavaConstants.NAME_SPACE_TO_PACKAGE_OPTION,
							new CommandLineOption(
		...
		if (isServerSideInterface) {
			optionMap
					.put(CommandLineOptionConstants.WSDL2JavaConstants.SERVER_SIDE_INTERFACE_OPTION,
							new CommandLineOption(
	...
	public String getBaseUri(String wsdlURI) {
		...
		try {
			...
			if ("file".equals(url.getProtocol())) {
				baseUri = new File(url.getFile()).getParentFile().toURL()
						.toExternalForm();
			} else {
	 ...
	 * 
	 * @param wsdlURI
	 *            the filesystem location (full path) of the WSDL file to read
	 *            in.
	 * @return the WSDLDescription object containing the WSDL Object Model of
	 *         the given WSDL file
	 * @throws WSDLException
	 *             when WSDL File is invalid
	 * @throws IOException
	...
	public AxisService getAxisService(String wsdlURI) throws Exception {
		...
		// A better fix should be to determine which builder to use based on the
		// wsdl version.
		// The current implementation of the wsdl builder classes did not allow
		// for this. I will suggest
		// that the determination of which builder to use should be done in the
		// builder classes, preferably
		// in the parent builder class.
		// Accessable through a static reference to a method like
		// getBuilderInstance(String wsdlURI) in
		// the parent builder class or through a builder Abstract Factor or
		// Abstract factory methods.

		WSDL11ToAxisServiceBuilder builder = new WSDL11ToAxisServiceBuilder(url
