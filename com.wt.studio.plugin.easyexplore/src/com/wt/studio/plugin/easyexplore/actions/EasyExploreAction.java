package com.wt.studio.plugin.easyexplore.actions;
...
public class EasyExploreAction implements IObjectActionDelegate, IWorkbenchWindowActionDelegate
{
	...
	public void run(IAction action)
	{
		try {
			if ("unknown".equals(selected)) {
				MessageDialog.openInformation(new Shell(), "Easy Explore", "Unable to explore "
						+ selectedClass.getName());
				EasyExploreActivator.log("Unable to explore " + selectedClass);
				return;
			}
			File directory = null;
			...
			String target = EasyExploreActivator.getDefault().getTarget();

			if (!EasyExploreActivator.getDefault().isSupported()) {
				MessageDialog
						.openInformation(
								new Shell(),
								"Easy Explore",
			...
			try {
				EasyExploreActivator.log("running: " + target);
				Runtime.getRuntime().exec(target);
			} catch (Throwable t) {
				MessageDialog.openInformation(new Shell(), "Easy Explore", "Unable to execute "
						+ target);
				EasyExploreActivator.log(t);
			}
		} catch (Throwable e) {
