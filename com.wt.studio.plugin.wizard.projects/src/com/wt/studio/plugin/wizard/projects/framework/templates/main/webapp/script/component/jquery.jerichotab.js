...
$.extend($.fn, {
    initJerichoTab: function(setting) {
        ...
        function createJerichoTab() {
            ...
            */  
            var jerichotab = $('<div class="jericho_tab"><div class="tab_pages" ><div class="tabs"><ul /></div></div><div class="tab_content"><div id="jerichotab_contentholder" class="content" style="overflow-y:hidden;height:100%;"/></div></div>')
                                                            ...
                                                            .applyHover()
                                                                .applyCloseEvent()
                                                                        .animate({ 'opacity': '1', width: opts.tabWidth }, function() {
																			//bug，如果初始化时有多个tab页，页签会跳到最后一个，注释了就好了.
																			//默认活动的为当前tab页中第一个。12/27
																			
																			//bug，注释掉会影响addTab方法，在addTabs中添加一个配置type
																			//用来表示是初始化还是点击添加
																			//如果是初始化，激活第0页，如果是添加，激活当前新tab页    12/30
																			if (ps.type == 'add') {
																				$.fn.setTabActive(curIndex);
																			}else if(ps.type == 'init') {
																				$.fn.setTabActive(0);
																			}
                                                                        });
    ...
    buildIFrame: function(src) {
        return this.each(function() {
            ...
            var iframe = $(' <iframe id="' + jerichotabiframe + '" name="' + jerichotabiframe + '" src="' + src + '" frameborder="0"  />')
    ...
    loadData: function() {
        return this.each(function() {
            ...
            if (holder.length == 0) {
                ...
                holder = $('<div  style="height:525px;"  class="curholder" id="' + holderId.replace('#', '') + '" />').appendTo($('#jerichotab_contentholder'));
