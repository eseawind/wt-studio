...
public class HorizontalFillLayout extends ToolbarLayout
{
	...
	public void layout(IFigure parent)
	{
		...
		for (int i = 0; i < numChildren; i++) {
			...
			if (child instanceof GhostModelFigure) {
				Dimension first = new Dimension(-1, clientArea.height);
				Point firstloc = new Point(clientArea.x, clientArea.y);
				Rectangle firstNewBounds = new Rectangle(firstloc, first);
				child.setBounds(transposer.t(firstNewBounds));
			} else {
				Point loc = new Point(x, y + 4);
				Dimension size = new Dimension(-1, clientArea.height - 8);
				Rectangle newBounds = new Rectangle(loc, size);
				int divided = (clientArea.width - 2 - ((numChildren - 2) * spacing))
						/ (numChildren - 1);
				if (i == numChildren - 1)
					divided = clientArea.width - 2 - ((divided + spacing) * (numChildren - 2));
